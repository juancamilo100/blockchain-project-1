...
class Blockchain {
    ...
    _addBlock(block) {
        return new Promise(async (resolve, reject) => {
            try {
                const lastBlock = this.height === -1 ? { hash: null } : this.chain.at(-1)
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if(elapsedTime > fiveMinutes) {
                ...
                try {
                    ...
                    const addedBlock = await this._addBlock(newBlock);
                    resolve(addedBlock)
                } catch (error) {
                    reject(error)
                }
    ...
    getBlockByHash(hash) {
        let self = this;
        return new Promise((resolve, reject) => {
            const foundBlock = this.chain.find((block) => {
                return JSON.stringify(block.hash) === JSON.stringify(hash);
            })
            resolve(foundBlock)
        });
    }
    ...
    getStarsByWalletAddress (address) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            for (let i = 1; i < this.chain.length; i++) {
                ...
                if(blockBody.data.owner === address) {
